...
主代理负责编排；**探索类工作须派子代理完成**（探索要读大量代码，上下文隔离价值高）。实现类工作默认派子代理，但改动很小且主代理直接执行不会明显吃上下文时，可走 trivial 豁免（见 Step 3）。
...
探索结论足以动手时，先按下方「trivial 豁免」判据决定由谁动手——满足豁免则主代理直接实现，否则走子代理路径（下列 1–6）。
...
**trivial** 指那种改动位置已知、不用再读一堆代码、验证输出就一两行的简单改动——既不需要上下文隔离、又不需要并行，主代理直接改完更快。

判据是 **主代理直接执行不会消耗大量上下文**（探索结论已经把上下文吃进来了，实现改动如果很小，再读和编辑不会明显膨胀），须同时满足：
